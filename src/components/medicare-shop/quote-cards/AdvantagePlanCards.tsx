...
export interface AdvantageCarrierSummary {
  id: string;
  name: string;
  logo: string;
  ...
  planName?: string;
  medicalDeductible?: string; // raw string description
...
interface Props {
  carriers: AdvantageCarrierSummary[];
  loading: boolean;
  onOpenCarrierDetails?: (carrier: AdvantageCarrierSummary) => void;
}

export const AdvantagePlanCards: React.FC<Props> = ({ carriers, loading, onOpenCarrierDetails }) => {
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {carriers.map(c => {
        const saved = isSaved(c.id, 'MA', 'advantage');
            ...
            <CardShell ref={ref as any} highlight={false} className="p-4 sm:p-5" key={c.id}>
              ...
              <div className="relative z-10 flex items-end justify-between mt-auto">
                <div className="flex flex-col">
                  ...
                  {c.moop && (
                    <div className="text-[11px] text-slate-400 mt-1">MOOP {c.moop}</div>
                  )}
                  {c.medicalDeductible && (
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Med Deductible {c.medicalDeductible}</div>
                  )}
                  {c.drugDeductible && (
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Drug Deductible {c.drugDeductible}</div>
                  )}
                ...
                <DetailsButton
                  onClick={() => {
                    ...
                    if (onOpenCarrierDetails) {
                      ...
                      const params = new URLSearchParams({ carrier: c.name, view: 'plan-details', category: 'advantage' });
...
export default AdvantagePlanCards;
