...
export interface CancerCarrierSummary {
  id: string; name: string; logo: string; min?: number; max?: number; planRange?: {min:number; max:number; count:number}; planName?: string; lumpSum?: number; wellness?: number; recurrence?: boolean; count: number;
}
interface Props { carriers: CancerCarrierSummary[]; loading: boolean; onOpenCarrierDetails?: (c: CancerCarrierSummary) => void; }

const CancerPlanCards: React.FC<Props> = ({ carriers, loading, onOpenCarrierDetails }) => {
            ...
            <div className={`relative z-10 flex flex-col h-full transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <div className="relative z-10 flex items-end justify-between mt-auto">
                <div className="flex flex-col">
                  ...
                  {c.wellness != null && <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Wellness ${c.wellness}</div>}
                  {c.recurrence && <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Recurrence Benefit</div>}
...
export default CancerPlanCards;
