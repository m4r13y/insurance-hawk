...
export const AgeBasedRateChart: React.FC<AgeBasedRateChartProps> = ({ ageIncreases, currentAge }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
  <div className="bg-white dark:bg-slate-900/90 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg p-3">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex bg-gray-100 dark:bg-slate-700/60 rounded-lg p-1 border border-gray-200 dark:border-slate-600/50">
            <button
        ...
        {chartType === 'line' && (
          <div className="flex items-center space-x-4 text-xs">
            <div className="flex items-center">
              ...
              <span className="text-gray-600 dark:text-slate-400">Annual Increase</span>
            </div>
            <div className="flex items-center">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
        {chartData.map((item, index) => (
          <div key={index} className="text-center p-2 bg-gray-50 dark:bg-slate-800/60 rounded border border-gray-200 dark:border-slate-700/60">
            <div className="text-gray-600 dark:text-slate-400">{item.year}</div>
            <div className="font-medium text-blue-600 dark:text-blue-400">{item.increaseLabel}</div>
            <div className="text-gray-500 dark:text-slate-500 text-xs">Age {item.age}</div>
          </div>
        ))}
      </div>

      {/* Additional Insights */}
      <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-400/30 rounded-lg p-3">
        <h6 className="text-sm font-medium text-blue-900 dark:text-blue-300 mb-2">Key Insights</h6>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-blue-800 dark:text-blue-300/90">
          <div>
