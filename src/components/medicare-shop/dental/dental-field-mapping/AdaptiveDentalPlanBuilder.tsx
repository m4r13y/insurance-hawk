...
const getCachedLogoUrl = (carrierName: string): string => {
  const tempQuoteForCarrierInfo = { carrier: { name: carrierName } };
  const enhancedInfo = getEnhancedCarrierInfo(tempQuoteForCarrierInfo, 'dental');
  return enhancedInfo.logoUrl;
};

const getCarrierDisplayName = (carrierName: string): string => {
  return getCarrierDisplayNameFromSystem(carrierName, 'dental');
};
    ...
    <div>
        ...
        <div className="flex items-center justify-between mb-6 p-4 bg-blue-50 rounded-lg border">
          <div className="flex items-center gap-3">
            <div className="flex-shrink-0 w-10 h-10 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
              <Image
                src={getCachedLogoUrl(selectedCompany)}
                alt={`${getCarrierDisplayName(selectedCompany)} logo`}
                width={40}
                height={40}
                ...
                onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                  ...
                  if (parent) {
                    ...
                    const initials = getCarrierDisplayName(selectedCompany)
                      .split(' ')
            ...
            <div>
              <h3 className="font-semibold text-lg">{getCarrierDisplayName(selectedCompany)}</h3>
              <p className="text-sm text-gray-600">Selected Carrier</p>
            </div>
          </div>
          <Button
            variant="outline"
      ...
      <div className="space-y-6">
            ...
            {!selectedCompany && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {companiesList.map((company) => {
                  ...
                  const displayName = getCarrierDisplayName(company);
                  const logoUrl = getCachedLogoUrl(company);
                  const subsidiaryName = getSubsidiaryName(company, 'dental');
                  
                  // Get AM Best rating from first quote
                  const representativeQuote = companyQuotes[0];
                  const amBestRating = representativeQuote?.ambestRating;
                  
                  return (
                    <Card 
                      key={company}
                      className="cursor-pointer hover:shadow-lg transition-shadow duration-200 border-1 hover:border-primary-300"
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-xs">
                              {companyQuotes.length} options
                            </Badge>
                          </div>
                          <div className="flex items-center gap-1 text-sm text-gray-600">
                            <DollarSignIcon className="h-3 w-3" />
                            <span>${priceRange.min} - ${priceRange.max}/month</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
          ...
          {selectedCompany && patternAnalysis && !finalQuote && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Left Column: Annual Maximum Selection */}
              <div>
                <Card>
                  <CardHeader>
                    <CardTitle>1. Choose Coverage Level</CardTitle>
                    <p className="text-sm text-gray-600">Select your preferred annual maximum</p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {Array.from(new Set(availableQuotes.map(q => q.annualMaximum)))
                        .sort((a, b) => a - b)
                        .map((max) => {
                          const quotesForMax = availableQuotes.filter(q => q.annualMaximum === max);
                          const priceRange = {
                            min: Math.min(...quotesForMax.map(q => q.monthlyPremium)),
                            max: Math.max(...quotesForMax.map(q => q.monthlyPremium))
                          };
                          ...
                          return (
                            <div 
                              ...
                              className={`flex items-center justify-between p-4 border rounded-lg cursor-pointer hover:border-primary transition-colors ${
                                selectedAnnualMax === max ? 'border-primary bg-primary-50' : 'border-gray-200'
                              }`}
                              onClick={() => {
                                setSelectedAnnualMax(max);
                                handleConfigurationChange('annualMaximum', max);
                              }}
                            >
                              <div className="flex items-center gap-3">
                                <input
                                  type="radio"
                                  checked={selectedAnnualMax === max}
                                  onChange={() => {}}
                                  className="text-primary"
                                />
                                <div>
                                  <div className="font-medium">${max.toLocaleString()} Annual Maximum</div>
                                  <div className="text-sm text-gray-600">{quotesForMax.length} plan{quotesForMax.length > 1 ? 's' : ''} available</div>
                                </div>
                              </div>
              ...
              {/* Right Column: Plan Features Overview */}
              <div>
                <Card>
                  <CardHeader>
                    <CardTitle>Plan Features Overview</CardTitle>
                    <p className="text-sm text-gray-600">
                      {selectedAnnualMax 
                        ? `Benefits for $${selectedAnnualMax.toLocaleString()} Annual Maximum`
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {selectedAnnualMax ? (
                        // Show actual benefits for selected annual maximum
                        (() => {
                          const selectedQuotes = availableQuotes.filter(q => q.annualMaximum === selectedAnnualMax);
                          const representativeQuote = selectedQuotes[0];
                          
                          if (!representativeQuote) {
                            return <div className="text-gray-500">No plan details available</div>;
                          }

                          // Parse benefit notes to extract coverage details
                          const benefitNotes = representativeQuote.benefitNotes || '';
                          
                          return (
                            <div className="bg-blue-50 p-4 rounded-lg">
                              <h4 className="font-medium text-blue-800 mb-3">Selected Plan Benefits</h4>
                              ...
                              <div className="space-y-3">
                                  ...
                                  <div className="bg-white rounded border p-3 text-xs">
                                    <div className="grid grid-cols-2 gap-2">
                                      <div>
                                        ...
                                        <p>${representativeQuote.annualMaximum.toLocaleString()}</p>
                                      </div>
                                      <div>
                                        <span className="font-medium">Monthly Premium:</span>
                                        <p className="text-green-600 font-medium">${representativeQuote.monthlyPremium}/month</p>
                                      </div>
                                      <div className="col-span-2">
                                        <span className="font-medium">Plan Name:</span>
                                        <p>{representativeQuote.fullPlanName}</p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div>
                                  <h5 className="font-medium text-sm mb-2">Coverage Benefits:</h5>
                                  <div className="bg-white rounded border p-3 text-xs">
                                    <div className="whitespace-pre-wrap">
                                      {benefitNotes.length > 0 ? benefitNotes.substring(0, 500) + (benefitNotes.length > 500 ? '...' : '') : 'Benefit details available upon selection'}
                                    </div>
                                  </div>
                                </div>
                                ...
                                {selectedQuotes.length > 1 && (
                                  <div className="bg-yellow-50 p-2 rounded text-xs">
                                    <span className="font-medium text-yellow-800">
                                      {selectedQuotes.length} plan options available with this annual maximum
                                    </span>
                                  </div>
                                )}
                              </div>
                        ...
                        // Show real patterns from available quotes when no annual maximum selected
                        (() => {
                          const sampleQuote = availableQuotes[0];
                          if (!sampleQuote) {
                            return <div className="text-gray-500">No plan details available</div>;
                          ...
                          return (
                            <div className="space-y-4">
                              ...
                              {/* Variables */}
                              <div>
                                <h4 className="font-semibold text-blue-700 mb-2">What Varies</h4>
                                <div className="bg-blue-50 p-3 rounded-lg space-y-2">
                                  <div className="text-sm">
                                    <span className="font-medium text-blue-800">Annual Maximum:</span> Choose from available options
                                  </div>
                                  <div className="text-sm">
                                    <span className="font-medium text-blue-800">Monthly Premium:</span> Varies by annual maximum
                                  </div>
                                  {availableQuotes.length > 1 && (
                                    <div className="text-sm">
                                      <span className="font-medium text-blue-800">Plan Options:</span> {availableQuotes.length} different configurations available
                                    </div>
                                  )}
                                </div>
                              </div>
          ...
          {/* Clean Two-Column Layout when Final Quote is Available */}
          {finalQuote && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Left Column: Plan Details + Button */}
              <div className="space-y-4">
                {/* Plan Details Card */}
                <Card>
                  <CardHeader>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium">{finalQuote.companyName}</p>
                        </div>
                        <div>
                          <span className="text-gray-600">Plan:</span>
                          <p className="font-medium">{finalQuote.fullPlanName}</p>
                        </div>
                        <div>
                          <span className="text-gray-600">Annual Maximum:</span>
                          <p className="font-medium">${finalQuote.annualMaximum.toLocaleString()}</p>
                        </div>
                        <div>
                          <span className="text-gray-600">Monthly Premium:</span>
                          <p className="font-bold text-green-600 text-lg">${finalQuote.monthlyPremium}/month</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          onClick={() => {
                            // Only reset plan-level selections, keep carrier
                            setSelectedAnnualMax(null);
                            setConfiguration({});
                            setFinalQuote(null);
                          }}
              ...
              {/* Right Column: Plan Benefits Overview */}
              <div>
                <Card>
                  <CardHeader>
                    <CardTitle>Plan Features Overview</CardTitle>
                    <p className="text-sm text-gray-600">What stays the same vs what varies</p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {/* Constants */}
                      ...
                      <div>
                        ...
                        <div className="bg-blue-50 p-3 rounded-lg space-y-2">
                          <div className="text-sm">
                            <span className="font-medium text-blue-800">Annual Maximum:</span> ${finalQuote?.annualMaximum.toLocaleString() || 'Choose from available options'}
                          </div>
                          <div className="text-sm">
                            <span className="font-medium text-blue-800">Monthly Premium:</span> ${finalQuote?.monthlyPremium || 'Varies by annual maximum'}
                          </div>
                          {availableQuotes.length > 1 && (
                            <div className="text-sm">
                              <span className="font-medium text-blue-800">Plan Options:</span> {availableQuotes.length} different configurations available
                      ...
                      {/* Plan Limitations */}
                      {(() => {
                        const sampleQuote = finalQuote || availableQuotes[0];
                        if (!sampleQuote?.limitationNotes) {
                          // Fallback limitations content when no real data is available
                          return (
                            <div>
