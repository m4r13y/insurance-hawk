...
export const EnrollmentPeriodsTool: React.FC = () => {
  ...
  function getIEPResult(birthDate: Date, note: string) {
    ...
    return `
      <div class='grow text-left'>
        <div class='font-semibold text-foreground text-lg mb-2'>Initial Enrollment Period (IEP)</div>
        <div class='font-semibold text-success text-lg mb-1'>${format(startIEP)} – ${format(endIEP)}</div>
        <div class='text-sm text-muted-foreground'>Enroll in Part A, Part B, Part D, or Medicare Advantage. This is your Initial Enrollment Period (IEP), a 7-month window based on your 65th birthday.</div>
        ${note}
      </div>
    `;
  }
  ...
  function getMedigapResult(birthDate: Date) {
    ...
    return `
      <div class='grow text-left'>
        <div class='font-semibold text-foreground text-lg mb-2'>Medigap Open Enrollment</div>
        <div class='font-semibold text-success text-lg mb-1'>${format(eligibleMonth)} – ${format(medigapEnd)}</div>
        <div class='text-sm text-muted-foreground'>Best time to get a Medicare Supplement without underwriting. This is your Medigap Open Enrollment, a 6-month window starting with your Part B eligibility.</div>
      </div>
    `;
  }
  ...
  function showEnrollmentWindows() {
    ...
    if (birthDate.getDate() === 1) {
      ...
      note = "<p class='text-sm text-muted-foreground'>* Since your birthday is on the 1st, Medicare considers you eligible a month earlier.</p>";
    }
    ...
    <>
      <div className="max-w-md mx-auto p-6 bg-card rounded-xl shadow-md border border-border mt-4">
        ...
        <div className="mb-2">
          ...
          />
          {dobError && <div className="text-destructive text-sm mt-1">{dobError}</div>}
        </div>
            ...
            <div className="bg-card border border-border rounded-xl shadow-2xs pointer-events-auto">
              <div className="p-6 sm:p-7">
                <div className="text-center">
                  <h3 id="enroll-modal-label" className="block text-xl sm:text-2xl font-semibold text-foreground my-2">Your Medicare Enrollment Periods</h3>
              ...
              <div className="flex justify-end items-center gap-x-4 p-6 sm:px-7">
                ...
                <button type="button" className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-primary text-primary-foreground hover:bg-primary/90 focus:outline-hidden focus:bg-primary/80 disabled:opacity-50 disabled:pointer-events-none" onClick={() => alert('You will be reminded at your enrollment period!')}>
                  Remind Me
                </button>
              </div>
            </div>
