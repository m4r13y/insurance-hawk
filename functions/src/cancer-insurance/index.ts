...
export const getCancerInsuranceQuote = functions.https.onCall(
  async (
    ...
    try {
      ...
      const inputVariablesRef = dbFirestore
        .collection("bflic-cancer-quotes")
        .doc("input-variables");
      const statesRef = dbFirestore
        .collection("bflic-cancer-quotes")
        .doc("states");

      functions.logger.info(
        "2. Fetching config documents: 'input-variables' and 'states'."
      );
      ...
      if (!rateDocSnap.exists) {
        functions.logger.error(
          `Rate document not found at path: ${rateDocPath}`
        );
        throw new functions.https.HttpsError(
          "not-found",
